...
pub fn export_embeddings(embeddings: &HashMap<String, Vec<f32>>, path: impl AsRef<Path>) -> Result<()> {
    if let Some(parent) = path.as_ref().parent() {
        fs::create_dir_all(parent)?;
    }
    // Convert Vec<f32> to strings for compact JSON
    let encoded: HashMap<String, String> = embeddings.iter()
        .map(|(k, v)| (k.clone(), v.iter().map(|f| f.to_string()).collect::<Vec<_>>().join(",")))
        .collect();
...
pub fn import_embeddings(path: impl AsRef<Path>) -> Result<HashMap<String, Vec<f32>>> {
    ...
    Ok(encoded.into_iter()
        .map(|(k, v)| (k, v.split(',').filter_map(|s| s.parse::<f32>().ok()).collect()))
        .collect())
}
